...
grid.arrange(
  ...
  ggplot() +
    ...
    geom_sf(data = atl_msa, fill = 'transparent', color = 'black', linewidth = 0.5) +
    scale_fill_manual(values = c('#5475A8','#ff0000','#38814E','#FBF65D','#C8E6F8','#A3CC51'), na.value = "transparent", name = 'Land Cover') +
    theme_void() +
...
#write.csv(lc2011_extract, "lc2011_extract.csv", row.names = FALSE)

#lc2021_extract <- terra::extract(lc2021_class,fishnet,fun=table) 
#write.csv(lc2021_extract, "lc2021_extract.csv", row.names = FALSE)


lc2011_extract <- read.csv("lc2011_extract.csv") %>%
  mutate(pct_developed = Developed / (Developed + Water + Forest + Farm + Wetlands + OtherUndeveloped) * 100)

lc2021_extract <- read.csv("lc2021_extract.csv") %>%
...
ggplot(data=fishnet_pct_developed)+
  ...
  scale_fill_viridis(option='cividis',name='Percent Developed')+
  geom_sf(data=atl_msa,fill='transparent',color='white',linewidth=0.5)+
  theme_void()+
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle('2011 Percent Developed 2011'),
...
ggplot(data=fishnet_pct_developed)+
  ...
  theme_void()+
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle('2021 Percent Developed ')
)
...
ggplot(data=fishnet_pct_developed)+
  ...
  scale_fill_manual(values=c('gray95','#B50000'),labels=c('Not Developed','Developed'),name='')+
  geom_sf(data=atl_msa,fill='transparent',color='black',linewidth=0.5)+
  theme_void()+
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggplot(data=fishnet_pct_developed)+
  ...
  scale_fill_manual(values=c('gray95','#38814E'),labels=c('No Change','Change'),name='')+
  geom_sf(data=atl_msa,fill='transparent',color='black',linewidth=0.5)+
  theme_void()+
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggplot() +
  ...
  scale_fill_viridis_c(option = 'plasma', name = 'Spatial Lag to Development (feet)') +  # Using 'plasma'
...
ggplot() +
  ...
  scale_fill_viridis(option='cividis',name='Population 2011',direction=-1)+
  labs(title="Atlanta MSA Population: 2011") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_void(),

ggplot() +
  geom_sf(data = atlantaPop21, aes(fill=pop_2021), colour=NA) +
  scale_fill_viridis(option='cividis',name='Population 2021',direction=-1)+
  labs(title="Atlanta MSA Population: 2021") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_void(), ncol=2)
```
...
ggplot()+
  ...
  scale_fill_viridis_c(option = 'cividis', name='Distance to Highway in feet')+
...
ggplot(data=testSetProbs,aes(d=as.numeric(class),m=probs))+
  geom_roc(n.cuts=50,color='lightblue',labels=FALSE)+
  annotate("text", x = 0.1, y = 1, label=paste("AUC: ",as.character(auc)),color='lightblue')+
  theme_bw()
...
ggplot()+
  geom_sf(data=fishnet_2021_dev,fill='gray90',color='transparent')+
  geom_sf(data=fishnet_2021_results,aes(fill=outcome),color='transparent')+
  geom_sf(data=atl_msa,color='gray90',linewidth=0.5,fill='transparent')+
  scale_fill_manual(values=c('#38814E','#B50000'),name='Predicted Outcome',labels=c('No Development Change','Development Change'))+
