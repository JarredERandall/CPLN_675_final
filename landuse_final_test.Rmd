...
AtlantaMSA <- 
  st_read('Data/atlanta_msa_bound.geojson') %>%
  st_transform('EPSG:2240') # JE Note: added 'ESRI:' in front of projection


lc_2011 <- raster('Data/lc_2011.tif')

lc_2011 <- project(lc_2011,'EPSG:2240')




lc_2021 <- raster('Data/lc_2021.tif')

lc_2021 <- project(lc_2021,'EPSG:2240')
...
# Assuming lc_2001 and lc_2011 are already loaded SpatRasters
developed_2011 <- classify(lc_2011, reclassMatrix)
developed_2021 <- classify(lc_2021, reclassMatrix)

# Adding the classified rasters
development_change <- developed_2011 + developed_2021

# Create a histogram of the change data
hist(values(development_change), main="Land Cover Change Distribution", xlab="Change Type", breaks=3, col="skyblue")
...
```{r, warning = FALSE, message = FALSE}
# Only keep cells that changed from undeveloped to developed (1)
development_change <- ifel(development_change == 1, 1, NA)

library(ggplot2)

# Convert development_change to a raster that can be used in ggplot
development_change_df <- as.data.frame(development_change, xy=TRUE, na.rm=TRUE) %>%
  rename(lc_change = Layer_1)

ggplot() +
  geom_sf(data=AtlantaMSA) +
  geom_raster(data=development_change_df, aes(x=x, y=y, fill=factor(lc_change))) +
  scale_fill_viridis_d(name="Land Cover Change") +
...
Next, the fishnet is created at 2500 by 2500 foot resolution and subset it to the Houston MSA with `st_intersection`. 
...
```{r, warning = FALSE, message = FALSE}
changePoints <- as.points(development_change, values = TRUE)

# Convert the SpatVector to an sf object directly
changePoints_sf <- st_as_sf(changePoints)

# Aggregate data by unique polygon ID
changePoints_sf <- st_set_crs(changePoints_sf, st_crs(AtlantaMSA_fishnet)) %>%
  rename(lc_change = Layer_1)

fishnet_checked <- st_join(AtlantaMSA_fishnet, changePoints_sf, join = st_intersects) %>%
  group_by(geometry) %>%
  summarise(lc_change = sum(lc_change, na.rm = TRUE), .groups = 'drop')

# Check summaries to see if the counts seem reasonable
summary(fishnet_checked$lc_change)

fishnet_aggregated <- fishnet_checked %>%
  mutate(lc_change = ifelse(lc_change > 0, 1, 0),
         lc_change = as.factor(lc_change))

table(fishnet_aggregated$lc_change)  # See the distribution of change

ggplot() +
  geom_sf(data=AtlantaMSA_fishnet, fill="white", color="black") +
  geom_sf(data=fishnet_aggregated, aes(fill=lc_change), color="black", size=0.1) +
  scale_fill_manual(values = c("grey", "red"),
                    labels = c("No Change", "New Development"),
                    name = "Land Cover Change") +
  labs(title = "Verified Land Cover Development Change", subtitle = "Visualized over Fishnet") +
  theme_minimal() +
...
```{r load_key_hide, warning= FALSE, include=FALSE}4
...
census_api_key("5d1422e560c28f68bcd7be7673cca0c77cd40c21", overwrite = TRUE)
```
...
```{r, warning = FALSE, message = FALSE, results = "hide"}
AtlantaHighways <-
  st_read('Data/atlanta_highway.geojson') %>%
